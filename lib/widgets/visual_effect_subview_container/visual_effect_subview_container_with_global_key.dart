...
/// As a user of the [flutter_acrylic] package it is recommended to use that widget
...
class VisualEffectSubviewContainerWithGlobalKey extends StatefulWidget {
  ...
  static const bottomRightCorner = VisualEffectSubviewProperties.bottomRightCorner;
  static const bottomLeftCorner = VisualEffectSubviewProperties.bottomLeftCorner;

  const VisualEffectSubviewContainerWithGlobalKey({required GlobalKey key, required this.child, this.alphaValue = 1.0, this.cornerRadius, this.cornerMask = 0xf, required this.effect, required this.state, required this.padding, this.resizeEventRelay}) : super(key: key);

  @override
  State<VisualEffectSubviewContainerWithGlobalKey> createState() => _VisualEffectSubviewContainerWithGlobalKeyState();
}

class _VisualEffectSubviewContainerWithGlobalKeyState extends State<VisualEffectSubviewContainerWithGlobalKey> {
  ...
  /// the Swift side.
  void _modifyVisualEffectSubview({required double xPosition, required double yPosition, required double width, required double height}) {
    if (_visualEffectSubviewId == null) {
      return;
    }
    ...
    if (!delta.isEmpty) {
      Window.updateVisualEffectSubviewProperties(_visualEffectSubviewId!, delta);
  ...
  void _updateVisualEffectSubview() {
    final renderObject = (widget.key as GlobalKey).currentContext!.findRenderObject() as RenderBox;
    final position = renderObject.localToGlobal(Offset.zero);
    
    final windowHeight = MediaQuery.of(context).size.height;
    
    final xPosition = position.dx + widget.padding.left;
    final yPosition = windowHeight - renderObject.size.height - position.dy + widget.padding.bottom;
    final width = renderObject.size.width - widget.padding.left - widget.padding.right;
    final height = renderObject.size.height - widget.padding.bottom - widget.padding.top;
